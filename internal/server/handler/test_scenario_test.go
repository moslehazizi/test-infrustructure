...
func TestTestScenarioHandler_New(t *testing.T) {
...
func TestTestScenarioHandler_Create(t *testing.T) {
	t.Run("success_case_bad_values_set", func(t *testing.T) {
		...
		r := chi.NewRouter()
		r.Post("/test-scenarios", handler.Create)
		...
		reqBody := `{
			...
			"num_steps": 2,
			"increase_agent_number": 2,
			"execution_number_multi_agent": 2,
			"test_service_config": {
				"increase_fixed_input": 100,
				"execution_number_multi_fixed_input": 20,
				"max_requests": 1,
				"max_duration": 1,
		...
		sampleTestScenarioRequest := &entity.TestScenario{
			...
			TestServiceConfig: &entity.TestServiceConfig{
				IncreaseFixedInput:     100,
				ExecNumMultiFixedInput: 20,
	...
	t.Run("success_case_dont_fill_null_fields_in_test_scenario", func(t *testing.T) {
		mockSvc := new(mocks.MockTestScenario)
		handler := NewTestScenarioHandler(mockSvc)
	...
	t.Run("failed_case_validation_error_handler", func(t *testing.T) {
		...
		assert.Equal(t, pkg.MaxTestServiceCountLessThanOne, result.Error)
		mockSvc.AssertExpectations(t)
	})
}
...
func TestTestScenarioHandler_GetPaginated(t *testing.T) {
	t.Run("success_case_with_nil_values", func(t *testing.T) {
		...
		r := chi.NewRouter()
		r.Post("/test-scenarios/search", handler.GetPaginated)
...
func TestTestScenariosHandler_Update(t *testing.T) {
	t.Run("success_case_bad_values_set", func(t *testing.T) {
		...
		r := chi.NewRouter()
		r.Post("/test-scenarios/update", handler.Update)
